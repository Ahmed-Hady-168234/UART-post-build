...
{
	uint32  int_type ;
	int_type = (uint8)((uint32_BaseAddress[UART1] & 0x0F0) >> 4);
...
{
	uint8  int_type;
	int_type = (uint8)((uint32_BaseAddress[UART2] & 0x0F0) >> 4);
	switch(int_type)
	    {
        case 0:
            /*Transmit Interrupt Clear*/
...
{
	uint8  int_type;
	int_type = (uint8)((uint32_BaseAddress[UART3] & 0x0F0) >> 4);
	switch(int_type)
	    {
    case 0:
        /*Transmit Interrupt Clear*/
...
{
	uint8  int_type;
	int_type = (uint8)((uint32_BaseAddress[UART4] & 0x0F0) >> 4);
	switch (int_type)
	    {
        case 0:
            /*Transmit Interrupt Clear*/
        ...
        case 1 :
            ...
            InterruptReceivedFlagCounter[UART4]++;
        break;
	  }
}
...
{
	uint8  int_type;
	int_type = (uint8)((uint32_BaseAddress[UART5] & 0x0F0) >> 4);
	switch(int_type)
	    {
    case 0:
        /*Transmit Interrupt Clear*/
...
{
	uint8  int_type;
	int_type = (uint8)((uint32_BaseAddress[UART6] & 0x0F0) >> 4);
	switch(int_type)
	    {
    case 0:
        /*Transmit Interrupt Clear*/
...
{
	uint8  int_type;
	int_type = (uint8)((uint32_BaseAddress[UART7] & 0x0F0) >> 4);
	switch(int_type)
	    {
    case 0:
        /*Transmit Interrupt Clear*/
...
{
    uint8 *_ptr_Buffer         = gArrRxRequest->ptr_Buffer;
	...
	case INTERRUPT_LEVEL_REACHED:	
		...
		/**/
           	...
           	else if( _Num_bytesToReceive == CounterBytesReceived[channelNum] )
	        {
           	...
           	{
           		ReciecveStates = NUM_OF_REMINDED_BYTES_LESS_THAN_INTERRUPT_LEVEL;
           	}
	break;
